...
namespace Latihan_5_1
{
    public partial class Form1 : Form
    {
        ...
        private void rtbNotepad_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                rtbNotepad.ContextMenu = contextMenu;
            }
        }
        bool cut_action = false;
        ...
        {
            Clipboard.SetText(rtbNotepad.SelectedText);
        }

        void PasteAction(object sender, EventArgs e)
        {
            int start = rtbNotepad.SelectionStart;
            if (Clipboard.ContainsText())
            {
                rtbNotepad.Text += Clipboard.GetText(TextDataFormat.Text).ToString();
                int length = Clipboard.GetText(TextDataFormat.Text).ToString().Length;
                if (cut_action == true) { Clipboard.Clear(); cut_action = false; }
                rtbNotepad.SelectionStart = start+ length;
            }
        }  

        void DeleteAction(object sender, EventArgs e)
        {
            rtbNotepad.SelectedText = "";
        }
